...
{
    int address = (int)strtol(string, NULL, 16);
...
void debugger_popup()
{
        ImGui::SetNextWindowSize(ImVec2(200 * scaling, 400 * scaling));
        ...
        ImGui::Text("add breakpoint %02d", numBreakpointsInUse);
        if (ImGui::Button("Add"))addBreakpoint(buf);
        ImGui::InputText("address", buf, IM_ARRAYSIZE(buf));

        // Display contents in a scrolling region
        ImGui::TextColored(ImVec4(1,1,0,1), "Breakpoints");
